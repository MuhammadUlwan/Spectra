  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-indigo-100 p-4">
    <div class="flex flex-col md:flex-row w-full max-w-4xl bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <div class="md:w-2/5 bg-gradient-to-br from-green-600 to-indigo-700 text-white p-8 flex flex-col justify-center items-center">
        ...
        <p class="text-sm opacity-90 mb-6 text-center">Buat akun baru dengan kode referral</p>
      ...
      <!-- Kanan - Form Register Referral -->
      <div class="md:w-3/5 p-8 md:p-10">
        <h2 class="text-2xl md:text-3xl font-bold mb-2 text-gray-800 text-center">Daftar dengan Referral</h2>
        <p class="text-gray-600 text-sm md:text-base mb-6 text-center">Isi formulir berikut untuk membuat akun baru</p>
        ...
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div>
            <label class="block text-gray-700 text-sm font-medium mb-1">Kode Referral</label>
            <input type="text" v-model="form.referral_code" placeholder="Masukkan kode referral"
              class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
              :class="{'border-red-500': errors.referral_code}" @input="clearError('referral_code')"
            />
            <p v-if="errors.referral_code" class="text-red-500 text-xs mt-1">{{ errors.referral_code }}</p>
          </div>
...
const submit = () => {
  ...
  if(!form.referral_code) errors.referral_code = 'Kode referral harus diisi'
  ...
  setTimeout(() => {
    ...
    alert('Akun dengan referral berhasil dibuat! (Demo)')
  }, 1200)
}
</script>
